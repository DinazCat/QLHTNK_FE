  ...
  const validateForm = () => {
    if (
      ...
      if (parseInt(formState.SL) <= 0) {
        ...
        setErrors(
          "Ngày tháng năm sử dụng phải là ngày trong quá khứ hoặc hiện tại."
        );
        return false;
      } else {
        setErrors("");
  ...
  return (
    ...
    >
      <div className="col-sm-4 modal1">
        <form>
          <div className="mb-2">
            <b>Mã vật tư thiết bị</b>
          </div>
          <Select
            className="mb-2"
            value={
              materials.find((item) => item.maVt === formState.maVatTu) || ""
            }
            onChange={(value) =>
              value !== null
                ? setFormState({
                    ...formState,
                    maVatTu: value.maVt,
                    tenVatTu: value.tenVt,
                    soLuongTonKho: value.soLuongTonKho,
                  })
                : setFormState({ ...formState, maVatTu: "", tenVatTu: "" })
            }
            options={materials}
            isClearable
