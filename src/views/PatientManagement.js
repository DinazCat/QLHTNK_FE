...
const PatientManagement = (props) => {
    ...
    const [nhasi, setNhaSi] = useState(user?.loaiNguoiDung === 'Phụ tá' ? [] : user?.ten);
    ...
    const ThanhTien = () => {
        ...
        for (let i = 0; i < cthsdt.Thuoc.length; i++) {
            tien = tien + parseInt(cthsdt.Thuoc[i].Gia)*parseInt(cthsdt.Thuoc[i].SL)
        }
        return tien
    }
    ...
    const validateForm = () => {
        if (cthsdt.MaNhaSi != '' && cthsdt.ChanDoan != '' && cthsdt.DichVu.length > 0) {
            return true
        }
        else {
            let errorFields = [];
    ...
    const saveCTHSDT = async () => {
        ...
        if (selectedRecord == null) {
            ...
            const res = await api.createCTHSDT(data.ChiTietHsdt,data.DichVus,data.Thuocs,null,data.HoaDon)
            if(res?.message == undefined){
              ...
              handleShowDialog(`Error adding treatment detail! ${res?res?.message:""}`)
            }
        }
        else {
          const dichvuList = cthsdt.DichVu.map((item) => ({
                ...
                {user?.loaiNguoiDung === "Nha sĩ" && (
                  <div className="col-md-auto">
                    <input
                      ...
                      id="MaNV"
                      name="MaNhaSi"
                      value={state === "create" ? user?.ten : cthsdt.maNhaSiNavigation.tenNv
                      }
                    />
                  </div>
                )}
        ...
        {medicineModalOpen && (
          <FormRecordMedicine
            ...
            medicines={medicines.filter(medicine => 
              !cthsdt.Thuoc.some(selectedMedicine => selectedMedicine.maThuoc === medicine.maThuoc))}
