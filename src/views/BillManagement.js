...
const BillManagement = (props) => {
  ...
  const validateForm = () => {
    if (formState.ngayThanhToan!='' && formState.soTienThanhToan!=0) {
        ...
        setErrors(`Số tiền thanh toán không được lớn hơn số tiền cần phải trả là: ${conNo}`);
        return false;
      }
      else {
        setErrors("");
      ...
      for (const [key, value] of Object.entries(formState)) {
        if (value == "") {
          switch (key){
            case 'ngayThanhToan': 
              errorFields.push("Ngày thanh toán"); break;
            case 'soTienThanhToan': 
              errorFields.push("Số tiền thanh toán"); break;
            default: break;         
          }
        }
      }
...
const handleSubmitthanhtoan = (e) => {
  ...
  const validSubmitData = () => {
    ...
    else if(conNo > 0 && (TongTienDV+TongTienThuoc)<5000000 || conNo > 0 && recentDiscount!=null && ThanhTienSauGiamGia<5000000){
  ...
  const handleSubmit = async () => {
    ...
    if (choice) {
        ...
        const obj1 = {
            ...
            SoTienConNo: conNo,
            TinhTrang:conNo==0?"Đã thanh toán":(conNo <(TongTienDV+TongTienThuoc)-SoTienGiam)?"Đang trả góp":null
        }
      ...
      <div style={{ minHeight: "630px" }}>
            ...
            <table className="table">
              ...
              <tbody>
                {bills?.map((item, index) => (
                  <tr
                    key={index}
                    onClick={() => setSelectedRowById(index, item.maCthsdtNavigation)}
                  >
                    <td>{index + 1}</td>
                    <td>{item.maHd}</td>
                    <td>{item.maBn}</td>
              ...
              <div className="mt-2 pe-2 ps-2" ref={componentToPrintRef}>
                ...
                <div className="mt-3 row">
                  ...
                  <div className='col-lg-4 col-md-auto mb-2'>
                    ...
                    {Math.abs(
                      new Date(
                        Date.now() - new Date(bills[selectedRow]?.maBnNavigation.ngaySinh).getTime()
                      ).getUTCFullYear() - 1970
                    )}
                  </div>
                  <div className='col-lg-4 col-md-auto mb-2'>
                      ...
                      CTHSDT.dichVuDaSuDungs.map((item, index) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td>{item.soLuong}</td>
                            {/* <td>{"cho tick"}</td> */}
                            {item.taiKham ? <td>{item.taiKham}</td> : <td>Không tái khám</td>}
                          </tr>
                        );
                      })
                    ) : (
                          ...
                          <tr key={index}>
                            <td>
                              <div>
                                <div>
                                  <b>
                                    {index + 1}/ {item.maThuocNavigation.tenThuoc}
                                  </b>
                                </div>
                                <div
                                  className="ms-3"
                ...
                <div className="mt-3">
                  ...
                  >
                    <tbody>
                      ...
                      <tr>
                        <th>Số tiền giảm:</th>
                        <th>
                          {SoTienGiam ? new Intl.NumberFormat("en-DE").format(SoTienGiam) : 0}
                        </th>
                      </tr>
                      {SoTienGiam ?<tr>
                  ...
                  <th>{new Intl.NumberFormat("en-DE").format(tongTienThanhToan)}</th>
                </tr>
        ...
        <td className="text-center align-middle">
          <div className="table-responsive mx-auto" style={{ maxWidth: "800px" }}>
            <table className="table table-borderless">
              <tbody>
                {CTTT !== null? CTTT?.map((item, index) => (
                  <tr key={index}>
                    <td>
                      <div>
                        <b>{index + 1}/{"Thanh toán lần " + (index + 1)}</b>
                      </div>
                    </td>
                    <td>{new Intl.NumberFormat("en-DE").format(item.soTienThanhToan)}</td>
                    <td>{"Ngày thanh toán: " + moment(new Date(item.ngayThanhToan)).format("DD/MM/YYYY")}</td>
                    {!disableDiscount&&<td className="fit">
                <span className="actions">
                    <BsFillTrashFill
                        className="delete-btn"
                        onClick={() => {
                                                      //đã thanh toán trừ ra, còn nợ tăng lên
                            setTongTienThanhToan(parseInt(formState.soTienThanhToan) - parseInt(item.soTienThanhToan))
                            setConNo(conNo+parseInt(item.soTienThanhToan))
                            setCTTT(CTTT?.filter(((_, idx) => idx !== index)))
                            setCTTTnew(CTTTnew?.filter(((item1, idx) => item1!== item)))
                ...
                {isModal && (
                  <tr>
                    <td colSpan="3">
                      <div className="col-sm-4 modal1 mx-auto">
                        <form>
                          ...
                          <div className="form-group">
                            <label htmlFor="soTienThanhToan">Số tiền thanh toán</label>
                            <input
                ...
                {selectedRow!=null && !isModal && bills[selectedRow].tinhTrang !="Đã thanh toán"? (
                  <tr>
                    <td colSpan="3" className="text-center">
                      ...
                      <tr>
                        <th> Còn nợ:</th>
                        <th>
                          {new Intl.NumberFormat("en-DE").format(conNo)}
                        </th>
                      </tr>
                    </tbody>
                  </table>
